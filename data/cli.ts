...
const notify = () => writeFileSync(database + ".revision", randomUUID() + "\n");
...
if (command === "restore") {
  ...
  notify();
  console.log(`Restored ${database}; existing databases are never overwritten`);
  ...
  try {
    ...
    if (existsSync(database))
      assert.ok(
        ...
        `SQLite dump drift (${command}). Run mise run data:dump, review, then stage ${dumpFile}`,
      );
    console.log(
      existsSync(database)
  ...
  try {
    if (command === "migrate") {
      ...
      const ids = values.all
        ? eligible
        : [required(values.latest ? eligible.at(-1) : values.season, "season")];
      ...
      switch (command) {
        ...
        case "export": {
          ...
          if (values.output) writeFileSync(values.output, json, { flag: "wx" });
