# Historical baseline notes — not current validation instructions

Preserved from `d93e393` before the accepted TanStack cleanup. Commands below
that name retired scripts require a historical checkout; do not run them against
the current app. Use [README.md](README.md) for supported validation commands.

The ECharts visual checkpoint is `chart-parity` (`c88c657`); the final active
ECharts application is `sqlite-content-store` (`7df53c8`). Retired source can be
retrieved from Git history. Recorded captures/results remain in `runs/`.

Retired: `chart-parity.mjs`, `stats-parity.mjs`, `stats-tooltips.mjs`,
`stats-keyboard.mjs`, `chart-image-labels.mjs`, `chart-resilience.mjs`, and
`data-corrections.mjs`. Current coverage is mapped in README.md; legacy slider,
ECharts formatter and internal SVG assertions are intentionally not retained.

## Original baseline / regression capture notes
